import { postEditPago } from "@/requests/reqPagos";
...
const EditPayment = () => {
  ...
  const { register, handleSubmit, formState: { errors }, setValue, watch } = useForm({
    defaultValues: {
      clientes: dataprev.cliente_pago.map(cliente => cliente.cliente_id),
      precioTotal: dataprev.precio_total,
      cuotaInicial: dataprev.cuota_inicial,
      saldo: dataprev.saldo,
      predio: dataprev.predio.id
    }
  });
  ...
  async function onSubmit(data) {
    ...
    setLoading(true);
    try {
      const resp = await postEditPago({id: dataprev.id, ...data});
        ...
        <form onSubmit={handleSubmit(onSubmit)}>
          ...
          <CardFooter className="flex justify-between">
            ...
            <Button type="submit" color="blue" disabled={loading}>
              {loading ? <Spinner color="green" /> : "Actualizar Pago"}
            </Button>
          </CardFooter>
          <ToastContainer />
        </form>
